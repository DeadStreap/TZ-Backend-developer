...
// Распределяем ключи по товарам (по ~4 ключа на товар)
function assignKeysToProducts(skuList: string[], allKeys: string[]): { sku: string; code: string }[] {
  const keysPerProduct = Math.floor(allKeys.length / skuList.length);
  const assigned: { sku: string; code: string }[] = [];

  let keyIndex = 0;
  for (const sku of skuList) {
    for (let i = 0; i < keysPerProduct && keyIndex < allKeys.length; i++) {
      assigned.push({ sku, code: allKeys[keyIndex] });
      keyIndex++;
    }
  }
...
async function main() {
  ...
  const assignedKeys = assignKeysToProducts(products.map(p => p.sku), keys);
  for (const { sku, code } of assignedKeys) {
